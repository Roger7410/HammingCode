...
class Map(object):
    ...
    def write_to_file(self):
        for i in range(self.reducer_count):
            pickle.dump(self.table[i], open(self.task_id+'_m'+str(self.mapper)+"_r"+str(i), "wb"))
...
class Reduce(object):
    ...
    def reduce(self, k, vlist):
        for mapper in vlist:
            for word in vlist[mapper]:
                if word in self.table:
                    self.emit(word, self.table[word] + vlist[mapper][word])
                else:
                    self.emit(word, vlist[mapper][word])
